...
class GameScreenorg11 extends StatefulWidget {
  final int initialPoints;

  const GameScreenorg11({super.key, this.initialPoints = 0});

  @override
  State<GameScreenorg11> createState() => _GameScreenState();
}

class _GameScreenState extends State<GameScreenorg11>
    with TickerProviderStateMixin {
  final int _currentIndex = 0;

  final List<Widget> _screens = [
    const GameScreenorg11(),
    const BoardScreen(),
    const Prize_screne(),
    const ProfileScreen(),
  ];
  ...
  void startFlying() {
    ...
    movementTimer = Timer.periodic(const Duration(milliseconds: 100), (timer) {
      setState(() {
        ...
        birdY = -250 * pow(progress, 1.5).toDouble(); // Smooth arc upwards
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    /// ✅ New: bird starts near the ground
    final birdBaseTop = screenHeight - 140;
